...
분석 기간은 연도, 월, 일을 따로 고릅니다. 긴 기간을 볼 때 연도를 바꾸기 쉽도록
브라우저 기본 날짜 선택기 대신 별도 입력칸을 둔 것입니다.
...
- `equity_curve.csv`: 날짜별 NAV, 일별 수익률, drawdown, 거래비용
...
- 초기 매수와 리밸런싱은 대상 자산 가격과 필요한 환율이 실제 관측된 날짜에만 실행합니다.
